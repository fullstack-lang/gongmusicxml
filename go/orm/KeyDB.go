...
type KeyPointersEncoding struct {
	...
	// field Key_octave is a slice of pointers to another Struct (optional or 0..1)
	Key_octave IntSlice `gorm:"type:TEXT"`
}
...
func (backRepoKey *BackRepoKeyStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, key *models.Key) (Error error) {
	...
	if keyDB, ok := backRepoKey.Map_KeyDBID_KeyDB[idx]; ok {
		...
		// 1. reset
		keyDB.KeyPointersEncoding.Key_octave = make([]int, 0)
		// 2. encode
		for _, key_octaveAssocEnd := range key.Key_octave {
			key_octaveAssocEnd_DB :=
				backRepo.BackRepoKey_octave.GetKey_octaveDBFromKey_octavePtr(key_octaveAssocEnd)
			
			// the stage might be inconsistant, meaning that the key_octaveAssocEnd_DB might
			// be missing from the stage. In this case, the commit operation is robust
			// An alternative would be to crash here to reveal the missing element.
			if key_octaveAssocEnd_DB == nil {
				continue
			}
			
			keyDB.KeyPointersEncoding.Key_octave =
				append(keyDB.KeyPointersEncoding.Key_octave, int(key_octaveAssocEnd_DB.ID))
		}
...
func (keyDB *KeyDB) DecodePointers(backRepo *BackRepoStruct, key *models.Key) {
	...
	// This loop redeem key.Key_octave in the stage from the encode in the back repo
	// It parses all Key_octaveDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	key.Key_octave = key.Key_octave[:0]
	for _, _Key_octaveid := range keyDB.KeyPointersEncoding.Key_octave {
		key.Key_octave = append(key.Key_octave, backRepo.BackRepoKey_octave.Map_Key_octaveDBID_Key_octavePtr[uint(_Key_octaveid)])
	}
