...
func (controller *Controller) GetPlayer(c *gin.Context) {
	...
	if err := db.First(&playerDB, c.Param("id")).Error; err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
...
func (controller *Controller) UpdatePlayer(c *gin.Context) {
	...
	// fetch the player
	query := db.First(&playerDB, c.Param("id"))
	...
	query = db.Model(&playerDB).Updates(playerDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
