...
type CreditPointersEncoding struct {
	...
	// field Bookmark is a slice of pointers to another Struct (optional or 0..1)
	Bookmark IntSlice `gorm:"type:TEXT"`

	// field Credit_image is a pointer to another Struct (optional or 0..1)
	// This field is generated into another field to enable AS ONE association
	Credit_imageID sql.NullInt64
}
...
func (backRepoCredit *BackRepoCreditStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, credit *models.Credit) (Error error) {
	...
	if creditDB, ok := backRepoCredit.Map_CreditDBID_CreditDB[idx]; ok {
		...
		// 1. reset
		creditDB.CreditPointersEncoding.Bookmark = make([]int, 0)
		// 2. encode
		for _, bookmarkAssocEnd := range credit.Bookmark {
			bookmarkAssocEnd_DB :=
				backRepo.BackRepoBookmark.GetBookmarkDBFromBookmarkPtr(bookmarkAssocEnd)
			
			// the stage might be inconsistant, meaning that the bookmarkAssocEnd_DB might
			// be missing from the stage. In this case, the commit operation is robust
			// An alternative would be to crash here to reveal the missing element.
			if bookmarkAssocEnd_DB == nil {
				continue
			}
			
			creditDB.CreditPointersEncoding.Bookmark =
				append(creditDB.CreditPointersEncoding.Bookmark, int(bookmarkAssocEnd_DB.ID))
		}

		// commit pointer value credit.Credit_image translates to updating the credit.Credit_imageID
		creditDB.Credit_imageID.Valid = true // allow for a 0 value (nil association)
		if credit.Credit_image != nil {
			if Credit_imageId, ok := backRepo.BackRepoImage.Map_ImagePtr_ImageDBID[credit.Credit_image]; ok {
				creditDB.Credit_imageID.Int64 = int64(Credit_imageId)
				creditDB.Credit_imageID.Valid = true
			}
		} else {
			creditDB.Credit_imageID.Int64 = 0
			creditDB.Credit_imageID.Valid = true
		}
...
func (creditDB *CreditDB) DecodePointers(backRepo *BackRepoStruct, credit *models.Credit) {
	...
	// This loop redeem credit.Bookmark in the stage from the encode in the back repo
	// It parses all BookmarkDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	credit.Bookmark = credit.Bookmark[:0]
	for _, _Bookmarkid := range creditDB.CreditPointersEncoding.Bookmark {
		credit.Bookmark = append(credit.Bookmark, backRepo.BackRepoBookmark.Map_BookmarkDBID_BookmarkPtr[uint(_Bookmarkid)])
	}

	// Credit_image field
	credit.Credit_image = nil
	if creditDB.Credit_imageID.Int64 != 0 {
		credit.Credit_image = backRepo.BackRepoImage.Map_ImageDBID_ImagePtr[uint(creditDB.Credit_imageID.Int64)]
	}
	return
}
...
func (backRepoCredit *BackRepoCreditStruct) RestorePhaseTwo() {
	...
	for _, creditDB := range backRepoCredit.Map_CreditDBID_CreditDB {
		...
		// reindexing Credit_image field
		if creditDB.Credit_imageID.Int64 != 0 {
			creditDB.Credit_imageID.Int64 = int64(BackRepoImageid_atBckpTime_newID[uint(creditDB.Credit_imageID.Int64)])
			creditDB.Credit_imageID.Valid = true
		}
